...
### Backtest : vectorisé par défaut

Le backtest de la boucle d'agent (`alpha_lab/pipeline.py`) est **vectorisé** —
le PnL quotidien est une seule opération matricielle (appartenance top-N de la
veille × rendements), donc une expérience tourne de bout en bout en **~2 s** au
lieu de plusieurs minutes. Il est sûr vis-à-vis du look-ahead mais **ignore les
coûts de transaction / le slippage**, donc le Sharpe rapporté est légèrement
optimiste — à utiliser pour *explorer* largement. Le chemin event-driven qf-lib
(`_run_qf_backtest`, avec commissions IB) est conservé pour la **validation
réaliste en coûts** d'une stratégie présélectionnée, et
`research/run_tsmom_backtest.py` l'utilise toujours pour les tearsheets du
dashboard. Le contrat de stratégie est inchangé : `signal(ctx)` renvoie une
matrice de scores `date × ticker`.
